...
total_spots_simulated = Value('i', 0)
total_spots_lock = Lock()
...
@njit()
def generate_surface_points(num_pts, number_observations, dtype=np.float32):
    '''
    Creates an array of points that are as equally spaced as possible on the 
    surface of a sphere using a Fibonacci spiral.
    ...
    number_observations : int
        Number of observations for one rotation of the star
    dtype : numpy data type, optional
        Data type to reduce memory usage (default: float32)
    ...
    indices = np.arange(num_pts, dtype=np.float64) + 0.5
    phi = np.arccos(1 - 2*indices/num_pts).astype(dtype)
    theta = (np.pi * (1 + 5**0.5) * indices).astype(dtype)
    ...
    thetas = np.arctan2(ys, xs).astype(dtype)
    phis = np.arcsin(zs).astype(dtype)
    
    
    # Manual repeat implementation
    phis_repeated = np.zeros((num_pts, number_observations), dtype=np.float64)
    thetas_repeated = np.zeros((num_pts, number_observations), dtype=np.float64)
    
    for i in range(num_pts):
        phis_repeated[i, :] = phis[i]
        thetas_repeated[i, :] = thetas[i]
    
    return thetas_repeated, phis_repeated

@njit()
def get_points_within_circ_on_sphere(centre_phi, centre_theta, radius, n=5e5,
                                     n_observations=72, thetas=None, phis=None, return_idx=False):
    # If thetas not provided, generate in-place to minimize memory allocation
    if thetas is None:
        # Use pre-allocated arrays to reduce memory churn
        n = int(n)
        thetas = np.empty(n, dtype=np.float64)
        phis = np.empty(n, dtype=np.float64)
        
        # In-place generation to avoid temporary allocations
        for i in range(n):
            z = 2 * np.random.random() - 1
            phi = 2 * np.pi * np.random.random()
            
            thetas[i] = np.arccos(z)
            phis[i] = phi
    
    # Ensure thetas and phis are 1D arrays (do this outside the function in real usage)
    assert thetas.ndim == 1, "thetas must be 1D"
    assert phis.ndim == 1, "phis must be 1D"
    
    # Preallocate output arrays with exact size needed
    # Precompute cos_ang_dist to avoid multiple allocations
    cos_ang_dist = np.empty_like(thetas, dtype=np.float64)
    
    # Trigonometric precomputation using existing references
    sin_centre_phi = np.sin(centre_phi)
    ...
    # Find indices within the radius - use numpy's boolean indexing for efficiency
    valid_mask = ang_dist < radius
    
    # Memory-efficient indexing
    valid_thetas = thetas[valid_mask]
    valid_phis = phis[valid_mask]
    valid_indices = np.where(valid_mask)[0]
    
    # Return based on flag with minimal additional allocations
    if return_idx:
        return valid_thetas, valid_phis, valid_indices
    
    return valid_thetas, valid_phis, np.array([-1], dtype=np.int64)
...
def get_analytic_paths(obs_phi, spot_phi, spot_theta, num_pts = 72, contrast = 1):
    ...
    obs_theta = np.arange(0, 2*np.pi, 2*np.pi/num_pts) + spot_theta[:, np.newaxis]

    spot_theta = 0                                           
    
    # Angle between centre of sphere and spot                                           
    delta = np.sin(obs_phi)*np.sin(spot_phi[:, np.newaxis]) +                                \
    np.cos(obs_phi)*np.cos(spot_phi[:, np.newaxis])*np.cos(spot_theta - obs_theta - np.pi/2)
    ...
    a = np.cos(spot_phi[:, np.newaxis]) # <--- Get radius of 'small' circle on sphere
    b = np.cos(spot_phi[:, np.newaxis])*np.sin(obs_phi) # <--- Projection effect of
                                         #     observation angle
    
    # Get x,y positions of spot as star rotates
    x_pos = a*np.cos(obs_theta)
    y_pos = b*np.sin(obs_theta) + np.sin(spot_phi[:, np.newaxis])*np.cos(obs_phi) # <--\ 
...
def multi_analytic_paths(radii, thetas, phis, contrast ,obs_phi, surface_thetas, surface_phis,
    ...
    for i in range(len(radii)):
        ...
        more_thetas, more_phis, new_idx = get_points_within_circ_on_sphere(phi, theta,
                                        radius, n_surface, 1, surface_thetas, surface_phis,
                                                                       return_idx=True)

        # checks to see if points have been used before
        unused_idx = [_ for _ in new_idx if _ not in unique_idx]
        
        # restrict more_thetas, more_phis to be in new_idx, but not unique_idx
        unused_idx_idx = np.where(~np.isin(new_idx, unique_idx))[0] # dumb name....
        more_thetas = more_thetas[unused_idx_idx]
        more_phis   = more_phis[unused_idx_idx]

        # add current things to unique points...
        unique_idx = np.unique(np.concatenate([unique_idx, new_idx]))
...
def get_data(radii, spot_thetas, spot_phis, contrast, obs_phi, n_observations, surface_thetas, surface_phis,
    ...
    true_x, true_y = multi_analytic_paths(radii, spot_thetas, spot_phis,
                                          contrast,obs_phi, surface_thetas, surface_phis, n_surface = num_pts,
...
@njit()
def get_spotted_idx(centre_phis, centre_thetas, radii, thetas, phis, n=5e5, n_observations=1):
    '''
    ===========================================================================
    Optimized function to compute indices of points within star-spot regions.
    ---------------------------------------------------------------------------
    centre_phis     - Array of latitudes of the center of the star-spots (radians)
    centre_thetas   - Array of longitudes of the center of the star-spots (radians)
    radii           - Array of radii of the star-spots (radians)
    thetas          - Array of theta values (radians) for points on the sphere
    phis            - Array of phi values (radians) for points on the sphere
    ...
    ---------------------------------------------------------------------------
    Returns:
        Unique indices of points that lie within any of the star-spots.
    ===========================================================================
    '''
    # Precompute trigonometric values for phis
    sin_phis = np.sin(phis)
    cos_phis = np.cos(phis)

    # Initialize a 1D boolean mask for valid points
    valid_mask = np.zeros(thetas.shape, dtype=np.bool_)  # Ensure consistent 1D shape

    for i in range(len(radii)):
        # Precompute trigonometric values for the current star-spot
        sin_centre_phi = np.sin(centre_phis[i])
        cos_centre_phi = np.cos(centre_phis[i])
    ...
    # Return the indices of valid points
...
def get_light_curve(obs_phi, all_phis, all_thetas, spotted_idx, num_pts=72, contrast=0.7):
    ...
    sin_obs_phi = np.sin(obs_phi)
    cos_obs_phi = np.cos(obs_phi)
    sin_all_phis = np.sin(all_phis)
    cos_all_phis = np.cos(all_phis)
    
    # Create observation angles more efficiently
    obs_theta = np.linspace(0, 2*np.pi, num_pts, endpoint=False)
    
    # Vectorized visibility calculation
    delta = (sin_obs_phi * sin_all_phis[:, np.newaxis] + 
             cos_obs_phi * cos_all_phis[:, np.newaxis] * 
             np.cos(all_thetas[:, np.newaxis] - obs_theta[np.newaxis, :] - np.pi/2))
    
    # Visibility mask
    visible = delta > 0
    
    # Vectorized position calculations
    x_pos = cos_all_phis[:, np.newaxis] * np.cos(all_thetas[:, np.newaxis] - obs_theta[np.newaxis, :])
    y_pos = cos_all_phis[:, np.newaxis] * np.sin(all_thetas[:, np.newaxis] - obs_theta[np.newaxis, :])
    
    # Pre-compute limb darkening
    dark = limb_darkening(x_pos, y_pos)
    
    # Initialize and compute brightness
    ...
    # Sum and normalize
    total_flux = np.sum(brightness, axis=0)
...
def process_single_rotation(args):
    print("process_single_rotation called with args:", args)

    """Helper function to process a single rotation for parallel processing"""
    n_spots, radii_method, radii_probs, latitude_method, obs_phi, n_observations, num_surf_pts, spot_contrasts, spot_ratio, return_Rper, surface_thetas, surface_phis = args
    ...
    # Get astrom data
    _, clean_data, _ = get_data(radii, spot_thetas, spot_phis, contrasts, obs_phi,
                               n_observations,  surface_thetas, surface_phis, num_pts=num_surf_pts,
                               verbose=False)
    ...
    # get Rper
    if return_Rper == True:
        spotted_idx = get_spotted_idx(spot_phis, spot_thetas, radii, surface_thetas, surface_phis, n = num_surf_pts)
        ...
        # but not in general
        lc = get_light_curve(obs_phi, surface_phis, surface_thetas, spotted_idx, 
                             100, contrast = 0.7)
...
def make_observations_parallel(n_rotations, num_spots, radii_method, radii_probs,
    ...
    surface_thetas, surface_phis = generate_surface_points(num_surf_pts, 1)
    surface_thetas = surface_thetas.ravel()
    surface_phis = surface_phis.ravel()
